...
var PORTAL_ROLE_MAP_ = { CEO: '社長', HQ: '本部', TEAM: 'マネージャー', TENCHO: '店舗' };
// TENCHO・マネージャーは「担当店舗（user_stores）」の範囲だけに絞る（本人の店舗以外は見せない）。
...
// 組み立てる。該当なし・無効化・役職未対応（社員/アルバイト等）・店舗未設定（TENCHO/TEAMで
...
function portalAutoAccount_(uid) {
  ...
  if (!role) return null;   // 社員・アルバイト等はダッシュボード対象外（従来どおり非対応）
