...
function supaLogin(p) {
  ...
  var email = '', uid = '';
  try { var u = JSON.parse(res.getContentText()); email = String(u.email || '').trim().toLowerCase(); uid = String(u.id || ''); } catch (e) {}
  ...
  // 2026-09-17追加（ユーザー要望「ポータルでアカウント発行したら役職ごとにダッシュボードにも
  // 自動で入れるようにしたい」）: アカウント管理シートに個別登録が無い場合、ポータル側の
  // Supabase（users.role・user_stores→stores.name）を正本として自動的に権限を組み立てる。
  // シート行が見つかった場合は上のループで既に返しているので、こちらは「シートに個別登録が無い」
  // ケースのフォールバックとして動く（＝手動で個別設定したアカウントは今まで通り最優先）。
  var portal = portalAutoAccount_(uid);
  if (portal) {
    sessionCleanup();
    var token2 = Utilities.getUuid();
    sessionPut(token2, portal);
    return { ok: true, token: token2, account: portal };
  }
  return { ok: false, error: 'このメール（' + email + '）に対応するダッシュボードアカウントがありません。アカウント管理の「メール」欄に登録するか、ポータル側で役職・担当店舗を設定してください' };
}

// ポータルの役職（users.role）→ダッシュボードの役職への対応表。PL・広告管理等の機密タブは
// ダッシュボード側のROLE_TABS/ROLE_FEATURESが既に役職別に制御しているため、ここでは
// 正しい役職にさえ変換すれば表示範囲は自動的に絞られる（店舗=PL非表示・広告管理非表示、等）。
// 2026-09-17: ユーザー指示によりSHAIN（社員）も店舗ロールとして自動連携（アルバイトは対象外のまま）。
var PORTAL_ROLE_MAP_ = { CEO: '社長', HQ: '本部', TEAM: 'マネージャー', TENCHO: '店舗', SHAIN: '店舗' };
// TENCHO・マネージャー・SHAINは「担当店舗（user_stores）」の範囲だけに絞る（本人の店舗以外は見せない）。
// CEO・HQは全社の役職のため、担当店舗の設定に関わらず「全店」固定（本部側の既存アカウントと同じ慣習）。
var PORTAL_ROLE_ALL_STORES_ = { CEO: true, HQ: true };

// uid（Supabase Auth・public.usersのid）から、ポータル役職→ダッシュボード権限のセッションを
// 組み立てる。該当なし・無効化・役職未対応（アルバイト等）・店舗未設定（TENCHO/TEAM/SHAINで
// 担当店舗0件）のいずれかならnullを返し、呼び出し側は通常の「アカウントが無い」エラーにする。
function portalAutoAccount_(uid) {
  if (!uid) return null;
  ...
  var headers = { apikey: supaKey, Authorization: 'Bearer ' + supaKey };
  var u;
  try {
    var ures = UrlFetchApp.fetch(supaUrl + '/rest/v1/users?select=id,name,role,is_active&id=eq.' + encodeURIComponent(uid), { headers: headers, muteHttpExceptions: true });
    if (ures.getResponseCode() !== 200) return null;
    var urows = JSON.parse(ures.getContentText() || '[]');
    if (!urows.length) return null;
    u = urows[0];
  } catch (e) { return null; }
  if (u.is_active === false) return null;
  var role = PORTAL_ROLE_MAP_[String(u.role || '').trim()];
  if (!role) return null;   // アルバイト等はダッシュボード対象外（従来どおり非対応）

  var storesLabel = '';
  if (PORTAL_ROLE_ALL_STORES_[String(u.role || '').trim()]) {
    storesLabel = '全店';
  } else {
    try {
      var sres = UrlFetchApp.fetch(supaUrl + '/rest/v1/user_stores?select=stores(name)&user_id=eq.' + encodeURIComponent(uid), { headers: headers, muteHttpExceptions: true });
      if (sres.getResponseCode() !== 200) return null;
      var srows = JSON.parse(sres.getContentText() || '[]');
      var names = srows.map(function (r) { return r.stores && r.stores.name; }).filter(Boolean);
      if (!names.length) return null;   // 担当店舗が1件も無い＝ポータル側の設定待ち（誤って全店を見せない）
      storesLabel = names.join('、');
    } catch (e) { return null; }
  }
  return { id: 'portal:' + uid, name: u.name || '', role: role, stores: storesLabel, tabs: '', perms: '', position: '', media: '' };
}
...
function bqGetReservation(p, session) {
  try {
    ...
    // 🚨応急の期間絞り込み（2026-09-02・実装指示書_表示集計層kdと高速化実行計画§6「一覧系APIは
    // limitと期間指定を必須」の方向性を先取り）: 全店・キャンセル込みの無期間クエリは91,801件
    // フルスキャン＋BigQueryページネーション約92回で、実測128〜145秒かつ時々GAS自体がタイムアウト
    // して失敗する（ユーザー報告「予約管理タブが読み込み中のまま止まる」で確認）。恒久対応は
    // kd_reservation_daily_summary（レーンP・W2）だが、それまでの間、from/toが指定されたら
    // visit_dateで絞り込み、対象行数を大きく減らしてタイムアウトを回避する。指定が無い呼び出し
    // （後方互換）は従来どおり無期間のまま＝挙動を変えない。
    var rsvFrom = /^\d{4}-\d{2}-\d{2}$/.test(String(p.from || '')) ? String(p.from) : '';
    var rsvTo = /^\d{4}-\d{2}-\d{2}$/.test(String(p.to || '')) ? String(p.to) : '';

    var ck = bqCacheKey_('reservation', [bqCacheGen_('reservation'), includeCancelled ? 'all' : 'active',
      restricted ? allowIds.slice().sort().join('.') : 'all', rsvFrom, rsvTo]);
    var cached = bqCacheGet_(ck);
    if (cached) return cached;
    ...
    if (rsvFrom) where.push("visit_date >= '" + rsvFrom + "'");
    if (rsvTo) where.push("visit_date <= '" + rsvTo + "'");
...
// p.rows = JSON文字列 [{store,date:'YYYY-MM-DD',values:[16個の数値],matchStore?}]（最大200件）。p.dryRun=true なら書き込まない。
// matchStore＝既存行を探すときの店舗名（cashReconReadで見えたシート上の表記。「うお蔵 新横浜店」等の表記ゆれで
// 別行として二重追加しないための指定。省略時はstoreで探す）。storeは新規追加する行に書く正式名。
...
function paymentDailyReplace(p) {
  ...
  try {
    ...
    for (var j = 0; j < req.length; j++) {
      var a = req[j], key2 = cashReconKey_(a.matchStore || a.store, a.date), hit = index[key2];
      if (hit && hit.length) {
...
// レジ照合で管理システムの現金売上が修正されたあと、精算対象店舗の精算書「N月現金売上」行を更新する中継。
// 精算ダッシュボード(別GAS)の sd_apiCashSync を、既存のPL自動連携と同じ SEISAN_WEBAPP_URL / PL_SYNC_TOKEN で呼ぶ。
// p.ym='YYYY-MM'、p.stores=精算システム側の店舗名のJSON配列。事前に rebuildAnalysis（分析_日別店舗の再生成）を済ませておくこと。
function seisanCashSync(p) {
  if (!cashReconAuth_(p)) return { ok: false, error: 'unauthorized' };
  var props = PropertiesService.getScriptProperties();
  ...
  var stores;
  try { stores = JSON.parse(p.stores || '[]'); } catch (e) { return { ok: false, error: 'stores がJSONとして読めません' }; }
  if (!Array.isArray(stores) || !stores.length) return { ok: true, results: ['対象店舗がありません'] };
  var res = UrlFetchApp.fetch(seisanUrl, {
    method: 'post', contentType: 'text/plain;charset=utf-8', muteHttpExceptions: true,
    payload: JSON.stringify({ fn: 'sd_apiCashSync', args: [plSyncToken, ym, stores] })
  });
  var j;
